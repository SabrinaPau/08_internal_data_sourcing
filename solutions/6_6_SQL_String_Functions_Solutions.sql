 ...
 */
SELECT faa || ' - ' || name AS faa_name
FROM airports;

SELECT CONCAT_WS(' - ', faa, name) AS faa_name
FROM airports;
...
SELECT COUNT(*)
FROM airports
WHERE SUBSTRING(name FROM 1 FOR 1) = SUBSTRING(city FROM 1 FOR 1);
 ...
 *    Please provide the query and answer below.
 */
SELECT SPLIT_PART(LEFT(lat::VARCHAR, POSITION('.' IN lat::VARCHAR) +1) || ';' ||
	   LEFT(lon::VARCHAR, POSITION('.' IN lon::VARCHAR) +1), ';', 1) AS lat,
	   SPLIT_PART(LEFT(lat::VARCHAR, POSITION('.' IN lat::VARCHAR) +1) || ';' ||
	   LEFT(lon::VARCHAR, POSITION('.' IN lon::VARCHAR) +1), ';', 2) AS lon
FROM airports;
 ...
 *     Please provide the query and answer below.
 */
SELECT DISTINCT REPLACE(country, 'land', 'sea')
FROM airports
WHERE country LIKE '%land%'
	  AND country NOT LIKE '%Island%';